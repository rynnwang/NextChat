...
6. **Build settings(构建设置)**:
   - **Build command(构建命令)**:请设置为 `yarn cf:build`(或 `npm run cf:build`)——
     **不要保留成 `yarn run build`/`next build`**。Cloudflare 的 Next.js 框架预设会自动填入普通的
     `build` 脚本,这个脚本只会执行 `next build`,不会触发 OpenNext 的转换步骤,导致 `.open-next/`
     目录始终不会生成。即使 Next.js 构建本身成功了,`wrangler deploy` 也会在最后一步失败,报错
     `ERROR Could not find compiled Open Next config, did you run the build command?`。如果你已经
     用自动填入的命令创建了 Worker,打开 **Build → Build configuration**(铅笔图标)修改这里的值,
     然后重新触发部署即可,不需要重新创建整个项目。
...
- **构建看起来成功了,但部署时报错 `ERROR Could not find compiled Open Next config, did you run
  the build command?`** —— 说明 **Build command** 被设置成了普通的 `yarn run build`/`next
  build`,而不是 `yarn cf:build`。请在该 Worker 的 **Build → Build configuration** 中修正,
  然后重新触发部署;详见上文第 2 节的提示。

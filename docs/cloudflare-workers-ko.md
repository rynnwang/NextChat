...
6. **Build settings(빌드 설정)**:
   - **Build command(빌드 명령어)**: `yarn cf:build`(또는 `npm run cf:build`)로 설정하세요 ——
     **`yarn run build`/`next build`로 그대로 두면 안 됩니다**. Cloudflare의 Next.js 프레임워크
     프리셋은 일반 `build` 스크립트를 자동으로 채워 넣는데, 이 스크립트는 `next build`만 실행할
     뿐 OpenNext 변환을 전혀 호출하지 않으므로 `.open-next/`가 생성되지 않습니다. 그 결과 Next.js
     빌드 자체는 성공하더라도, 마지막 단계에서 `wrangler deploy`가
     `ERROR Could not find compiled Open Next config, did you run the build command?` 오류로
     실패합니다. 이미 자동으로 채워진 명령어로 Worker를 생성했다면, **Build → Build
     configuration**(연필 아이콘)을 열어 값을 수정한 다음 배포를 다시 시도하세요 —— 프로젝트
     전체를 다시 만들 필요는 없습니다.
...
- **빌드는 성공한 것처럼 보였는데, 배포 단계에서 `ERROR Could not find compiled Open Next
  config, did you run the build command?` 오류가 발생함** —— **Build command**가
  `yarn cf:build`가 아니라 일반 `yarn run build`/`next build`로 설정되어 있는 것입니다. Worker
  설정의 **Build → Build configuration**에서 수정한 다음 배포를 다시 시도하세요. 위의 2절 안내도
  참고하세요.
